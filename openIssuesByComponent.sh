...
components=$(curl -s -i --header 'Private-Token: <privateToken>' "http://localhost/api/v4/projects/1/labels?scope=all&per_page=100" |grep name|     sed -e 's/[{}]/''/g' |      awk -v k="text" '{n=split($0,a,","); for (i=1; i<=n; i++) print a[i]}'|grep name|cut -d ':' -f 2|cut -d '"' -f 2)
